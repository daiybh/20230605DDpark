...
global_LastInfo = {"parkinfo": {},
                   "last_handle_query_token": {"json": {}, "time": 0, "count": 0},
                   "last_handle_chargeorder": {"json": {}, "time": 0, "count": 0}}
# Route to handle incoming data from the parking cloud
event = Event()


def updateLastHandleChargeorder(json):
    global global_LastInfo
    global_LastInfo['last_handle_chargeorder']['json'] = json
    global_LastInfo['last_handle_chargeorder']['time'] = datetime.now().strftime("%m/%d/%Y, %H:%M:%S %f")
    global_LastInfo['last_handle_chargeorder']['count'] += 1


def updateLastHandleQueryToken(json):
    global global_LastInfo
    global_LastInfo['last_handle_query_token']['json'] = json
    global_LastInfo['last_handle_query_token']['time'] = datetime.now().strftime("%m/%d/%Y, %H:%M:%S %f")
    global_LastInfo['last_handle_query_token']['count'] += 1
...
def handle_query_token():
    ...
    if retJson is None:
        ...
        updateLastHandleQueryToken(retJson)
        ret = 0
        app.logger.debug(
            f'{request.path},decocdeMessage data>>>{json.dumps(retJson,ensure_ascii=False)}')
...
def handle_chargeorder():
    ...
    if not auth or auth.type != 'bearer' or auth.token != config.shuyunInfo['token']:
        ...
        app.logger.debug(f'{request.path},auth[{auth}] failed>>>{message}  ,current token[{config.shuyunInfo["token"]}]')
    else:
...
# {"Data": "", "OperatorID": "10004", "TimeStamp": "1691039453", "Sig": "3B10EFC59AA7395F94B3E38BBA162CFD", "Seq": "107"}
        ...
        if a is None:
            app.logger.debug(
                f'{request.path},decocdeMessage failed>>>{message}')
        else:
            ret = 0
    ...
    def post(self, park_id):
        try:
            ...
            if request.values['actiontype'] == 'add':
                config.parkyunInfos[str(park_id)] = {
                    'park_id': park_id, 'ukey': ukey}
                config.writeJson()
